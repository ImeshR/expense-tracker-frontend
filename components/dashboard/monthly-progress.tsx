...
export function MonthlyProgress() {
    ...
    const percentage = Math.min(Math.round((currentMonthTotal / maxMonthlyExpense) * 100), 100)
    ...
    return (
        <div className="space-y-4">
            ...
            <Progress value={percentage} className="h-3 rounded-full" indicatorClassName={statusColor} />
            ...
            <div className="pt-4 space-y-2">
                <div className="flex justify-between mb-1 p-2 rounded-lg bg-background/50">
                    ...
                    <span className="text-sm font-medium">LKR {currentMonthTotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between mb-1 p-2 rounded-lg bg-background/50">
                    <span className="text-sm font-medium">Monthly Limit</span>
                    <span className="text-sm font-medium">LKR {maxMonthlyExpense.toLocaleString()}</span>
                </div>
                <div className="flex justify-between mb-1 p-2 rounded-lg bg-background/50">
                    <span className="text-sm font-medium">Remaining</span>
                    <span className="text-sm font-medium">LKR {(maxMonthlyExpense - currentMonthTotal).toLocaleString()}</span>
                </div>
            </div>
        </div>
    )
